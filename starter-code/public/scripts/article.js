...
(function(module) {
  ...
  */
  ...
  Article.allAuthors = () => {
    return Article.all
      .map(article => article.author) // dont need curly braces because article.author exists on the same line
        .reduce((acc, curr) => {
          if(!acc.includes(curr)) acc.push(curr) // if it returns true we will not fire it
          return acc
        }, [])
  };
...
// app.Article = Article (basically the same thing as the line above)
})(app)
